...
public class PopEffect : MonoBehaviour
{
    ...
    public void DisableAndPop()
    {
        ...
        popSequence.Append(targetObject.transform.DOScale(originalScale * .5f, popDuration).SetEase(Ease.OutQuad)).OnComplete(()=>{targetObject.SetActive(false);});   // Then scale back to its original size.
    }
